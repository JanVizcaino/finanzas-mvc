    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        <?php foreach($plans as $plan): ?>
            <a href="index.php?action=view_plan&id=<?= $plan['id'] ?>" class="block group">
                <div class="bg-white rounded-lg shadow-sm p-5 flex flex-col gap-2 h-full border border-transparent group-hover:border-primary/10 group-hover:shadow-md transition">
        ...
        <?php if (empty($plans)): ?>
            <div class="col-span-1 md:col-span-2 lg:col-span-3 text-center py-12 bg-white rounded-lg border border-dashed border-secondary/20 text-secondary">
...
<div id="slideOverBackdrop" class="fixed inset-0 z-50 invisible">
    ...
    <div class="fixed inset-y-0 right-0 flex max-w-full pl-0 sm:pl-10 pointer-events-none">
        ...
        <div id="slideOverPanel" class="pointer-events-auto w-screen sm:max-w-md transform translate-x-full transition-transform duration-300 ease-in-out bg-white shadow-xl flex flex-col h-full">
            ...
            <div class="h-auto sm:h-20 px-4 sm:px-8 py-4 sm:py-2.5 border-t border-secondary/20 flex flex-col-reverse sm:flex-row justify-end items-center gap-3 sm:gap-4 bg-white flex-shrink-0">
                <button type="button" onclick="closeSlideOver()" class="w-full sm:w-auto h-10 px-4 bg-white border border-secondary/30 hover:bg-secondary/5 rounded-md flex items-center justify-center text-secondary text-sm font-medium transition cursor-pointer">
                    Cancelar
                </button>
